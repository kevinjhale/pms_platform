...
export default function StripeSettingsForm({
  ...
  const [state, formAction, isPending] = useActionState(saveStripeSettingsAction, null);
  ...
  const handleTest = async (formData: FormData) => {
    ...
    if (!secretKey) {
      setTestResult({ valid: false, message: 'Enter a secret key to test' });
      return;
    }
    setIsTesting(true);
    setTestResult(null);
  ...
  const handleImport = async () => {
    setIsImporting(true);
    await importEnvSettingsAction('stripe');
  ...
  const handleClear = async () => {
    ...
    setIsClearing(true);
    await clearIntegrationSettingsAction('stripe');
    setIsClearing(false);
    window.location.reload();
  };
      ...
      >
        <div>
          ...
          <p style={{ fontSize: '0.875rem', color: 'var(--secondary)' }}>
            Accept online rent payments via Stripe
          </p>
        </div>
      ...
      <form action={formAction}>
        <div style={{ display: 'grid', gap: '1rem' }}>
          ...
          <FormField
            ...
            type="password"
            placeholder="whsec_..."
            helpText="For verifying webhook signatures (optional for local dev)"
          />
        </div>
